...
const BottomNav = ({ activeTab, onTabChange }) => {
    return (<nav className="fixed bottom-0 left-0 w-full flex justify-around py-4 bg-[#10141a] z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
...
const NavItem = ({ icon, label, active, onClick }) => {
    return (<motion.div whileHover={{ scale: 1.15, y: -2 }} whileTap={{ scale: 0.95 }} onClick={onClick} className={`flex flex-col items-center cursor-pointer ${active ? "text-green-400" : "text-gray-500 hover:text-gray-400"} transition-colors`}>
            <motion.div initial={false} animate={{ color: active ? "#4ade80" : "#6b7280" }} transition={{ duration: 0.2 }} className="mb-1">
                {icon}
            </motion.div>
